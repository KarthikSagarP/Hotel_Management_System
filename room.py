...
class roombooking:
    def __init__(self,root):
        ...
        #To fetch data 
        conn=mysql.connector.connect(host='localhost',user='root',password='C@pshe!1',database='hbms',auth_plugin='mysql_native_password')
        con_cursor=conn.cursor()
        con_cursor.execute('select RoomType from booking')
       ...
       #To fetch data 
        conn=mysql.connector.connect(host='localhost',user='root',password='C@pshe!1',database='hbms',auth_plugin='mysql_native_password')
        con_cursor=conn.cursor()
        con_cursor.execute('select RoomNo from room')
        ...
        subtatal=Label(labelframeleft,text='Sub Total:',padx=2,pady=6,font=('times new roman',12,'bold'))
        ...
        img3=Image.open(r'gui\bed room.jpg')
    ...
    def fetch_data(self):
        conn=mysql.connector.connect(host='localhost',user='root',password='C@pshe!1',database='hbms',auth_plugin='mysql_native_password')
        con_cursor=conn.cursor()
        con_cursor.execute('select contact,checkin,checkout,roomtype,roomavailable,meal,noofdays from booking')
        rows=con_cursor.fetchall()
        if len(rows)!=0:
